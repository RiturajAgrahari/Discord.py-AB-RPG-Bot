...
async def letter_event(uid, interaction):
    ...
    check_win_status = await select_query(column='letter_event', table="events", condition_column="uid", condition_value=int(uid))
    if check_win_status[0][0] == "won":
        await interaction.response.send_message("You already won the event", ephemeral=True)
    else:
